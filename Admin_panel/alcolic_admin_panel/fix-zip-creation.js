...
console.log('📦 Creating Fixed GitHub Package...');

// Create a working package with correct paths
function createWorkingPackage() {
  console.log('\n📁 Creating working package...');
  
  const packageDir = '/home/u294447786/domains/alcolic.gnritservices.com/public_html/alcolic-working-package';
...
function copyBackendFiles(packageDir) {
  ...
  exec(`mkdir -p ${backendDir}`, (error, stdout, stderr) => {
    ...
    // Copy backend files using rsync for better control
    exec(`rsync -av --exclude='node_modules' --exclude='logs' /home/u294447786/domains/alcolic.gnritservices.com/public_html/backend/ ${backendDir}/`, (error, stdout, stderr) => {
      if (error) {
        console.log('❌ Failed to copy backend files:', error.message);
        // Try simple copy instead
        exec(`cp -r /home/u294447786/domains/alcolic.gnritservices.com/public_html/backend/* ${backendDir}/ 2>/dev/null || true`, (error, stdout, stderr) => {
          console.log('✅ Backend files copied (simple method)');
          createFrontends(packageDir);
        });
      } else {
        console.log('✅ Backend files copied (rsync method)');
        createFrontends(packageDir);
      }
    });
  });
...
function createFrontends(packageDir) {
  console.log('\n🎨 Creating frontends...');
...
function createDocs(packageDir) {
...
function createZipFile(packageDir) {
  ...
  const zipName = 'alcolic-working-package.zip';
  const zipPath = '/home/u294447786/domains/alcolic.gnritservices.com/public_html/' + zipName;
  
  // Use zip command with correct path
  exec(`cd ${packageDir} && zip -r ${zipPath} .`, (error, stdout, stderr) => {
    if (error) {
      console.log('❌ Failed to create zip file:', error.message);
      // Try alternative method
      exec(`cd ${packageDir} && zip -r ../${zipName} .`, (error, stdout, stderr) => {
        if (error) {
          console.log('❌ Failed to create zip file (alternative):', error.message);
          return;
        }
        
        console.log('✅ Zip file created successfully (alternative method)!');
        showPackageInfo(zipPath);
      });
    } else {
...
function showPackageInfo(zipPath) {
  // Get file size
  exec(`ls -lh ${zipPath}`, (error, stdout, stderr) => {
    console.log('\n📊 Package Information:');
    console.log('='.repeat(50));
    console.log('📦 Package Name: alcolic-working-package.zip');
...
console.log('🚀 Starting working package creation...');
createWorkingPackage();
